...
export function createTheme(settings: SettingsState): Theme {
  const initialTheme = {
    ...
    cssVariables: {
      cssVarPrefix: "",
      // Keeps the same generated attribute selector the app already relies on
      // (see `stylesMode` in `src/theme/styles/utils.ts`).
...
export function createTheme(): Theme {
  ...
  const theme = muiCreateTheme(initialTheme, overridesTheme);
